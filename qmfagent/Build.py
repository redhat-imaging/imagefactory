...
class BuildAdaptor(object):
    ...
    def __init__(self, descriptor, target, image_uuid, sec_credentials):
		...
		if target == "mock":
			self._builder_class = MockBuilder.MockBuilder
		else: # otherwise, use the node value found in <os><name>ExampleOS</name></os> of the tdl.
			dom_tdl = minidom.parseString(descriptor)
			os_name = dom_tdl.getElementsByTagName("os")[0].getElementsByTagName("name")[0].childNodes[0].nodeValue
			class_name = os_name + "Builder"
			try:
				self._builder_class = getattr(os_name, os_name)
			except Exception, e:
